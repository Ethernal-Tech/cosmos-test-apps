...
CONSUMER_CHAIN_ID="gov-consumer"
MONIKER="testnet"
VALIDATOR="validator"
...
CONSUMER_REPRESENT1="representative1"
CONSUMER_REPRESENT2="representative2"
CONSUMER_DELEGATOR1="delegator1"
CONSUMER_DELEGATOR2="delegator2"
PROVIDER_HOME=$HOME/.tool_provider
PROVIDER_HOME1=$HOME/.tool_provider1
PROVIDER_NODE_ADDRESS="tcp://localhost:26658"
PROVIDER_NODE_ADDRESS1="tcp://localhost:26668"
...
./$CONSUMER_BINARY keys add $CONSUMER_REPRESENT1 $KEYRING --home $CONSUMER_HOME --output json > $CONSUMER_HOME/consumer_represent1_keypair.json 2>&1
./$CONSUMER_BINARY keys add $CONSUMER_REPRESENT2 $KEYRING --home $CONSUMER_HOME --output json > $CONSUMER_HOME/consumer_represent2_keypair.json 2>&1
./$CONSUMER_BINARY keys add $CONSUMER_DELEGATOR1 $KEYRING --home $CONSUMER_HOME --output json > $CONSUMER_HOME/consumer_delegator1_keypair.json 2>&1
./$CONSUMER_BINARY keys add $CONSUMER_DELEGATOR2 $KEYRING --home $CONSUMER_HOME --output json > $CONSUMER_HOME/consumer_delegator2_keypair.json 2>&1

# Add dinar to user account
./$CONSUMER_BINARY add-genesis-account $(jq -r .address $CONSUMER_HOME/consumer_keypair.json)  1000000000dinar --home $CONSUMER_HOME
./$CONSUMER_BINARY add-genesis-account $(jq -r .address $CONSUMER_HOME/consumer_represent1_keypair.json)  1000000000dinar --home $CONSUMER_HOME
./$CONSUMER_BINARY add-genesis-account $(jq -r .address $CONSUMER_HOME/consumer_represent2_keypair.json)  1000000000dinar --home $CONSUMER_HOME
./$CONSUMER_BINARY add-genesis-account $(jq -r .address $CONSUMER_HOME/consumer_delegator1_keypair.json)  1000000000dinar --home $CONSUMER_HOME
./$CONSUMER_BINARY add-genesis-account $(jq -r .address $CONSUMER_HOME/consumer_delegator2_keypair.json)  1000000000dinar --home $CONSUMER_HOME

# Add CONSUMER_REPRESENT1 as default admin
REPRESENTATIVE_USER_ADDRESS=$(jq -r .address $CONSUMER_HOME/consumer_represent1_keypair.json)
jq ".app_state.adminmodule.admins += [\"$REPRESENTATIVE_USER_ADDRESS\"]" $CONSUMER_HOME/config/genesis.json > \
      $CONSUMER_HOME/edited_genesis.json && mv $CONSUMER_HOME/edited_genesis.json $CONSUMER_HOME/config/genesis.json
sleep 1
...
# register some interchain account
# ./$PROVIDER_BINARY tx icamauth register --connection-id connection-0 \
#        --from $VALIDATOR \
#        $KEYRING \
#        --home $PROVIDER_HOME \
#        --node tcp://${PROVIDER_RPC_LADDR} \
...
# # register provider's validator ICA address as admin on consumer chain
# ICA_ADDRESS=$(./$PROVIDER_BINARY q icamauth interchainaccounts connection-0 $PROVIDER_VALIDATOR_ADDRESS --node tcp://${PROVIDER_RPC_LADDR} --output json)
# ./$CONSUMER_BINARY tx adminmodule add-admin $ICA_ADDRESS \
#        --from $CONSUMER_USER $KEYRING \
#        --home $CONSUMER_HOME \
#        --chain-id $CONSUMER_CHAIN_ID \
#        --node $CONSUMER_RPC_LADDR -y -b block
